...
int mbox_create(mbox **mb){
  struct mbox *new_mb = malloc(sizeof(mbox));
//  new_mb->msg = malloc(sizeof(struct messageNode));
//  new_mb->mbox_sem = malloc(sizeof(mbox->mbox_sem));
  //new_mb->msg = NULL;
  //new_mb->mbox_sem = NULL;
  *mb = new_mb;
  return 1;
}
...
void mbox_deposit(mbox *mb, char *msg, int len){
  struct messageNode *new_msg = malloc(sizeof(struct messageNode));
  struct messageNode *tmp = mb->msg;

  new_msg->message = malloc(len + 1);
  new_msg->message = msg;
  new_msg->len = len;
  new_msg->next = NULL;
...
void send(int tid, char *msg, int len){
	// struct messageNode *tmp = msgList->msg;
	// while(tmp != NULL){
	// 	if(tmp->sender == tid){
	// 		mbox_deposit(msgList, msg, len);
	// 	}
	// }
	...
	// tmp->next = NULL;
	struct mbox *msgList;
	while(msgList->msg != NULL){
		if(msgList->msg->sender == tid){
			break;
		}
		msgList->msg = msgList->msg->next;
	}
	mbox_deposit(msgList, msg, len);

	// struct messageNode *msgSend = malloc(sizeof(struct messageNode));
	// struct messageNode *tmp;
	// msgSend->message = malloc(len + 1);
	// strcpy(msgSend->message, msg);
	// msgSend->len = len;
	// msgSend->receiver = tid;
	// msgSend->sender = running->thread_id;
	// msgSend->next = NULL;
	// if(msgList->msg == NULL){
	// 	msgList->msg = msgSend;
	// }
...
void receive(int *tid, char *msg, int *len){
	struct mbox *receiveMail;
	while(receiveMail->msg != NULL){
		if(receiveMail->msg->receiver == tid){
			break;
		}
		receiveMail->msg = receiveMail->msg->next;
	}
	mbox_withdraw(receiveMail, msg, len);
	// struct messageNode *tmp = msgList->msg;
	// while(tmp != NULL){
	// 	if(tmp->receiver == tid){
	// 		mbox_withdraw(msgList, msg, len);
	// 	}
	// }
	// strcpy(tmp->message, msg);
	// tmp->len = len;
